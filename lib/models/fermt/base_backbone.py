...
from lib.models.fermt.RWKV_RTM import RWKV_RTM
...
class BaseBackbone(nn.Module):
    def __init__(self):
        ...
        self.dim_in = 192
        self.num_head = 12
        self.rwkv_rtm = RWKV_RTM(dim=self.dim_in, head=self.num_head)
    ...
    def forward_features(self, z, x_list, frame_id, update, mask=None):
        ...
        while num: # 1： test, >1:train
            ...
            tp, x, new_state_t, new_state_c = self.rwkv_rtm(z, x, self.state_t, self.state_c)
            self.state_t = new_state_t
            self.state_c = new_state_c
            x = combine_tokens(z, x, mode=self.cat_mode)
            x = torch.cat([tp, x], dim=1)
            x = self.pos_drop(x)
    ...
    def forward(self, z, x, frame_id, update, **kwargs):
        """
        ...
        x, aux_dict = self.forward_features(z, x, frame_id, update)
